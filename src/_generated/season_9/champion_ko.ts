...
export type Champion_season_9_ko = '트리스타나' | '이렐리아' | '아트록스' | '아리' | '아크샨' | '애쉬' | '아지르' | '벨베스' | '초가스' | '다리우스' | '에코' | '가렌' | '그웬' | '하이머딩거' | '자르반_4세' | '제이스' | '징크스' | '카이사' | '카르마' | '카타리나' | '클레드' | '크산테' | '리산드라' | '럭스' | '오리아나' | '뽀삐' | '쉔' | '렉사이' | '레넥톤' | '세주아니' | '세나' | '사이온' | '소나' | '소라카' | '스웨인' | '타릭' | '티모' | '벨코즈' | '워윅' | '야스오' | '제드' | '카시오페아' | '바이' | '마오카이' | '케일' | '사미라' | '우르곳' | '아펠리오스' | '제리' | '갈리오' | '칼리스타' | '나서스' | '말자하' | '탈리야' | '라이즈' | '세트' | '진' | '비에고' | '카사딘';
